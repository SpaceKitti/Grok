...
- **cmhd:** mode="cmhd" tracer + Russell e_int. Helmholtz ON (no acoustics, no nabla p). 3b: 2D bump rho=1+eps sin advects with solenoidal u; mean rho stays 1 with no pin/floor. 4: gamma=5/3, Dp/Dt=-gamma p div u +(gamma-1)Q (Q=viscous+Ohmic); E_int rises by heat only. mode="mhd" unchanged.
