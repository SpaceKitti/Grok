...
- **particles:** Stage 3 test-particle tracers (not PIC, not MMS). `mhd_params["particles"]=True` on mode=hall or twofluid Boris-pushes N_p=8 ions (q=+1, m=1) and N_p=8 electrons (q=-1, m_e=1/25) in the fluid Ohm E and spectral B after each fluid step. No current deposit, no back-reaction. KE_i / KE_e recorded separately. Smoke: `examples/test_particles.py`.
- **bulk zeta:** Stage 4, `mode="cmhd"` only. `mhd_params["zeta"]` (alias `bulk_zeta`) default 0 / off. When on: sigma_bulk = zeta (div u) I, dt u += grad(zeta div u) next to nu lap u (not /rho), Q += zeta (div u)^2. History `Q_bulk` = <zeta (div u)^2>. zeta=0 matches current cmhd. Hive GAMMA_DEFAULT stays 5/3. Not on mode=mhd / hall / twofluid. Do not retune the 1D nu lap u vs 2 nu S:S factor. Smoke: `examples/test_bulk_zeta.py`.
